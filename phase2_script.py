...
def CreateRecsIndex(sorted_filename):
	try:
		...
		commandForIdx = "awk -F: '{print $1; print $0}' < " + noBackSlashFile + " | db_load -T -t hash re.idx"
		print(commandForIdx)
...
def CreateTermsIndex(sorted_filename):
	try:
		...
		commandForIdx = "awk -F: '{print $1; print $0}' < " + noBackSlashFile + " | db_load -T -t btree te.idx"
		print(commandForIdx)
		os.system(commandForIdx)
	except:
		print("Could not create File with no Backslash or Index file could not be created.")
...
def CreateYearsIndex(sorted_filename):
	try:
		...
		commandForIdx = "awk -F: '{print $1; print $0}' < " + noBackSlashFile + " | db_load -T -t btree ye.idx"
		print(commandForIdx)
		os.system(commandForIdx)
	except:
		print("Could not create File with no Backslash or Index file could not be created.")
