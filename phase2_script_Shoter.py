...
def CreateIndex(sorted_filename):
	try:
		...
		if sorted_filename[2:3] == 'r':
			commandForIdx = "awk -F: '{print $1; print $0}' < " + noBackSlashFile + " | db_load -T -t hash " + IndexFilename
		else:
			commandForIdx = "awk -F: '{print $1; print $0}' < " + noBackSlashFile + " | db_load -T -t btree " + IndexFilename
		print(commandForIdx)
